...
Overall, loss to market giants and intense competition are the primary reasons for business failures, as cited more than 70% of startups. Poor market fit, stagnation post-acquisition and budget limits are other common reasons for start-up failures, regardless of sectors.

Besides, there are sector-specific attributes for these failures, signalling unique business practices across sectors.

- Accommodation and Food services startups reported 52% of their failures were due to High operational costs, which is unsurprising given the industry nature of heavy overheads into physical sites, e.g., hotels, restaurant.
- While competition in Healthcare is not as intense as other sectors, startups struggled with product-market fit, monetization failure and business execution. This tells a story of difficulties getting close to and empathize with customers, probably due to the industry nature of data confidentiality, discouraging people to share their health status.
- As an industry that requires significant upfront investment, Manufacturing startups faced a major financing challenges, with 55% reported No budget as their failure reason, followed by Execution flaws (28%).
...
#| fig-cap: "While low-funded startup (commonly seed rounds and series A) struggled with product-market fit and product scalability due to niche market, higher-funded ones (commonly series B and C) faced challenged in monetization, business executitions and post-acquisition operations. "
...
startup_combined <- startup_combined |> 
  mutate(funding_group = cut(funding_millions, breaks = c(-Inf, 20, 30, 70, Inf),   
        labels = c("(Pre-)Seed","Series A", "Series B", "Series C"))) 
...
While only funding amount is available in the dataset, we made an inference to funding rounds the startup has done to raise such amount of fundings. [explain a bit on the threshold defined for each funding round and what start-ups often do in each round]

Besides the universal attribution of competitive rivalries of major players, startups faced different challenges depending on their stage of life. 

- (Pre-)Seed startups are early-staged, often with just a proof-of-concept or minimal viable product, looking to raise funds for their product development and market validation. They are often not yet knowledgeable of how their products fit into the market, which explains why they hit a wall with Poor market fit (26%) or Niche market limits (29%)
