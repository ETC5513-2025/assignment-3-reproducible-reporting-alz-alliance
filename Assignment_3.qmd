...
To investigate the relationship between funding levels and failure reasons, funding data was cleaned by extracting numeric values and converted into millions for consistency across records. The dataset varaible description can be found @tbl-data-dic in the @Apendix section 

```{r tabular representation of funding raised by sector}
#| label: tbl-funding-sector
#| tbl-cap: Number of startup by sector and funding bracket


# Define funding brackets using the same funding_millions variable
...
@tbl-funding-sector provides an overview of the funds raised by companies in different sector. It is important to have an overview before we deep dive into the illustruative representational below which talks further on skew distribution.
